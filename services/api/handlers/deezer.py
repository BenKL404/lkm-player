...
async def download_playlist(playlist_id, retries=MAX_RETRIES):
    """Downloads all tracks from a Deezer playlist (same logic as album)."""
    playlist_info_attempt = 0
    playlist_tracks_infos = None
    tmp_download_dir = None

    while playlist_info_attempt < retries:
        try:
            playlist_tracks_infos = get_song_infos_from_deezer_website(
                "playlist", playlist_id
            )
            if not playlist_tracks_infos:
                raise ValueError(
            ...
            playlist_info_attempt += 1
            print(
                f"Attempt {playlist_info_attempt}/{retries}: Error fetching playlist info for {playlist_id}: {e}"
            )
            if playlist_info_attempt < retries:
                await maybe_refresh_deezer_session(
                    playlist_info_attempt, retries, f"playlist metadata {playlist_id}", e
                )
                await asyncio.sleep(1 * playlist_info_attempt)
            else:
                if tmp_download_dir and tmp_download_dir.exists():
                    await aioshutil.rmtree(tmp_download_dir, ignore_errors=True)
                raise

    if not playlist_tracks_infos or not tmp_download_dir:
        print(f"Failed to initialize playlist download for {playlist_id}.")
        return None

    downloaded_tracks_details = []
    tasks = []

    for i, track_infos in enumerate(playlist_tracks_infos):
        track_sng_id = track_infos.get("SNG_ID", f"playlist_{playlist_id}_track_{i}")
        file_extension, deezer_format = get_file_format(track_infos)
        song_path = tmp_download_dir / f"{track_sng_id}{file_extension}"
        ...
        async def download_single_with_retry(ti, fe, df, sp, track_retries=MAX_RETRIES):
            ...
            for attempt in range(track_retries):
                try:
                    ...
                    if "TRACK_NUMBER" in ti:
                        ti_copy["TRACK_NUMBER"] = ti["TRACK_NUMBER"]
                    print(f"Successfully downloaded track {track_id} to {sp}")
                    return ti_copy
                except Exception as track_e:
                    print(
                        f"Error downloading track {track_id} (attempt {attempt + 1}/{track_retries}): {track_e}"
                    ...
                    if attempt < track_retries - 1:
                        await maybe_refresh_deezer_session(
                            attempt + 1, track_retries, f"playlist track {track_id}", track_e
                        )
                        await asyncio.sleep(1 * (attempt + 1))
                    else:
                        return None
    ...
    if not downloaded_tracks_details:
        print(f"Failed to download any tracks for playlist {playlist_id}.")
        if tmp_download_dir and tmp_download_dir.exists():
            await aioshutil.rmtree(tmp_download_dir, ignore_errors=True)
        raise Exception(
            f"Failed to download any tracks for playlist {playlist_id} after retries."
        )
    ...
    print(
        f"Successfully downloaded {len(downloaded_tracks_details)} out of {len(playlist_tracks_infos)} tracks for playlist {playlist_id}"
    )
    return downloaded_tracks_details
...
def get_playlist_metadata_from_api(playlist_id):
    ...
    try:
        ...
        if "error" in tracks_json:
            print(
                f"API Error fetching tracks for playlist {playlist_id}: {tracks_json['error']}"
            )
            tracks_data = []
        else:
            tracks_data = tracks_json.get("data", [])
        ...
        cover_url = (
            playlist_json.get("picture_xl")
            or playlist_json.get("picture_big")
            or playlist_json.get("picture_medium")
            or playlist_json.get("picture")
            or ""
        )
        if cover_url:
            cover_response = deezer_download.session.get(cover_url, stream=True)
        ...
        creator_name = (
            playlist_json.get("creator", {}).get("name")
            or playlist_json.get("user", {}).get("name")
            or "Unknown"
        )
        playlist_title = playlist_json.get("title", f"Playlist {playlist_id}")
        playlist_link = playlist_json.get("link", DEEZER_URL + f"/playlist/{playlist_id}")

        clean_creator = clean_filename(creator_name)
        clean_playlist_title = clean_filename(playlist_title)
        ...
        metadata = {
            ...
            "release_date": "",
            "year": "",
            "album_link": playlist_link,
            "track_link": None,
            "cover_data": cover_data,
            "api_json": playlist_json,
            "tracks_api_data": tracks_data,
            "clean_artist": clean_creator,
            "clean_album_title": clean_playlist_title,
            "clean_title": clean_playlist_title,
        }
        print(
            f"Fetched metadata for playlist {playlist_id}: {creator_name} - {playlist_title}"
        )
        return metadata

    except requests.exceptions.RequestException as e:
        print(f"Network error fetching metadata for playlist {playlist_id}: {e}")
        raise
    except Exception as e:
        print(f"Error processing metadata for playlist {playlist_id}: {e}")
        raise


def get_playlist_caption(metadata):
    """Generates caption for a playlist."""
    return (
        "<b>Playlist: {title}</b>\n"
        "Créée par {artist}\n"
        '<a href="{album_link}">Lien Deezer</a>'
    ).format(**metadata)
...
async def send_playlist_audio(event: types.Message, metadata, dl_tracks_info):
    """Sends playlist tracks as audio files (same logic as album, with playlist caption)."""
    user_id, username, first_name = get_user_infos(event)
    print(
        f"USER_DEBUG: Sending playlist audio to user_id={user_id} username={username} first_name={first_name}"
    )
    caption = get_playlist_caption(metadata)
    if SEND_ALBUM_COVER and metadata.get("cover_data"):
        await event.answer_photo(
            BufferedInputFile(metadata["cover_data"], filename="cover.jpg"),
            caption=caption,
            parse_mode="HTML",
    ...
    for dl_info in dl_tracks_info:
        song_path = dl_info["song_path"]
        potential_id = Path(song_path).stem
        api_track = api_tracks_by_id.get(dl_info.get("SNG_ID")) or api_tracks_by_id.get(
            potential_id
        )
        if api_track:
            title = api_track.get("title", dl_info["song_name"])
            artists = [c["name"] for c in api_track.get("contributors", [])]
            performer = ", ".join(artists) if artists else metadata["artist"]
        else:
            title = dl_info["song_name"]
    ...
    if 2 <= len(media_group) <= 10:
        try:
            ...
            print(f"Failed to send playlist as media group, sending individually: {e}")
    ...
    for i, item in enumerate(processed_files):
        try:
            ...
            print(f"Error sending track {item['title']}: {e}")
            await event.answer(f"⚠️ Error sending track: {item['title']}")


async def create_and_send_zip(
    event: types.Message, metadata, dl_tracks_info, is_album: bool, is_playlist: bool = False
):
    """
    Creates a zip archive (single or multipart) and sends it.
    Handles both copying to a path and sending directly to Telegram.
    Places files inside 'Artist - Album [Year]' or 'Creator - Playlist' directory within the zip.
    """
    user_id, username, first_name = get_user_infos(event)
    print(
    ...
    year_suffix = f" [{metadata['year']}]" if metadata.get("year") else ""
    internal_dir_name = clean_filename(
        f"{metadata['clean_artist']} - {metadata['clean_album_title']}{year_suffix}"
    )
    ...
    base_name_title = metadata["title"]
    year_part = f" [{metadata['year']}]" if metadata.get("year") else ""
    final_title_for_hash = f"{metadata['artist']} - {base_name_title}{year_part}"
    if is_playlist:
        caption = get_playlist_caption(metadata)
    elif is_album:
        caption = get_album_caption(metadata)
    ...
    try:
        if metadata.get("cover_data"):
            ...
            await event.answer(caption, parse_mode="HTML")
    ...
    if COPY_FILES_PATH and FILE_LINK_TEMPLATE:
        ...
        _year_suffix = f" [{metadata['year']}]" if metadata.get("year") else ""
        output_base_path = Path(TMP_DIR) / clean_filename(
            f"{metadata['clean_artist']} - {metadata['clean_title']}{_year_suffix}"
        )
        zip_files_created = []
...
@deezer_router.message(F.text.regexp(PLAYLIST_REGEX))
async def handle_playlist_link(event: types.Message, real_link=None):
    """Handles Deezer playlist links."""
    user_id, username, first_name = get_user_infos(event)
    print(
        f"USER_DEBUG: Playlist link request from user_id={user_id} username={username} first_name={first_name}"
    )
    link_to_process = real_link or (event.text and event.text.strip()) or None
    if not link_to_process:
        await event.answer("Lien playlist invalide.")
        return
    playlist_match = re.search(PLAYLIST_REGEX, link_to_process)
    ...
    try:
        ...
        if dl_tracks_info and "download_dir" in dl_tracks_info[0]:
            ...
            download_dir_to_clean = Path(TMP_DIR) / "deezer" / "playlist" / str(playlist_id)

        metadata = get_playlist_metadata_from_api(playlist_id)

        if os.environ.get("FORMAT") == "zip":
            await create_and_send_zip(
                event, metadata, dl_tracks_info, is_album=False, is_playlist=True
            )
        else:
            await send_playlist_audio(event, metadata, dl_tracks_info)
        ...
        print(f"USER_DEBUG: Error processing playlist {playlist_id}: {e}")
        print(traceback.format_exc())
        await tmp_msg.delete()
        await event.answer(f"{__('download_error')} {str(e) or 'Erreur inconnue.'}")
    finally:
        remove_downloading(user_id)
        if download_dir_to_clean and download_dir_to_clean.exists():
            try:
                ...
                print(f"Error cleaning up directory {download_dir_to_clean}: {cleanup_e}")
